...
const FloorPlanEditor: React.FC = () => {
  ...
  const latestElementsRef = useRef<PlanElement[]>(floorPlanElements);
  ...
  const elementById = useMemo(
    () => new Map(floorPlanElements.map(element => [element.id, element] as const)),
    [floorPlanElements]
  );

  const cableRunByTargetId = useMemo(
    () => new Map(cableRuns.map(run => [run.toId, run] as const)),
    [cableRuns]
  );
  ...
  const voltageDropByElementId = useMemo(() => {
    if (viewMode !== 'voltage') {
    ...
    const getDistanceToSource = (elementId: string, depth = 0): number => {
      if (depth > 20) return 0;
      if (distanceCache.has(elementId)) {
        return distanceCache.get(elementId) ?? 0;
      }

      const run = cableRunByTargetId.get(elementId);
      if (!run) {
        distanceCache.set(elementId, 0);
        return 0;
      }

      const parentElement = elementById.get(run.fromId);
      if (!parentElement) {
        distanceCache.set(elementId, 0);
        return 0;
      }

      const distance = parentElement.type === 'db'
        ? run.length
        : run.length + getDistanceToSource(parentElement.id, depth + 1);

      distanceCache.set(elementId, distance);
      return distance;
    };

    const drops = new Map<string, number>();
    floorPlanElements.forEach(element => {
      if (!element.circuitId || element.type === 'db') {
        drops.set(element.id, 0);
        return;
      }

      const distance = getDistanceToSource(element.id);
      const section = element.type === 'light' ? 1.5 : 2.5;
      const power = element.power || (element.type === 'light' ? 0.1 : 3.5);
      drops.set(
        element.id,
        distance === 0 ? 0 : calculateVoltageDrop(distance, power, 230, section)
      );
    });

    return drops;
  }, [cableRunByTargetId, elementById, floorPlanElements, viewMode]);

  // --- History Management ---
  
  const saveHistory = (stateOverride?: Partial<HistoryState>) => {
    const currentState: HistoryState = {
      elements: stateOverride?.elements ?? floorPlanElements,
      walls: stateOverride?.walls ?? walls,
      cables: stateOverride?.cables ?? cableRuns
    };
  ...
  useEffect(() => {
    latestElementsRef.current = floorPlanElements;
  }, [floorPlanElements]);
  ...
  const applyPendingDrag = (
    elements: PlanElement[],
    pendingDrag: { id: string; x: number; y: number } | null
  ) => {
    if (!pendingDrag) {
      return elements;
    }
    ...
    const updatedElements = elements.map(element => {
      if (element.id !== pendingDrag.id) {
        return element;
      }
      if (element.x === pendingDrag.x && element.y === pendingDrag.y) {
        return element;
      }

      changed = true;
      return { ...element, x: pendingDrag.x, y: pendingDrag.y };
    });

    return changed ? updatedElements : elements;
  };
  ...
  const flushPendingDrag = () => {
    ...
    const updatedElements = applyPendingDrag(latestElementsRef.current, pendingDrag);
    if (updatedElements !== latestElementsRef.current) {
  ...
  const scheduleDragUpdate = (id: string, x: number, y: number) => {
    ...
    dragFrameRef.current = window.requestAnimationFrame(() => {
      ...
      setFloorPlanElements(prev => {
        const updatedElements = applyPendingDrag(prev, pendingDragRef.current);
        latestElementsRef.current = updatedElements;
        return updatedElements;
      });
      pendingDragRef.current = null;
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (mode === 'move' && dragId) {
       scheduleDragUpdate(dragId, finalX, finalY);
    }
  ...
  const handleMouseUp = () => {
    ...
    if (mode === 'move' && dragId) {
        const updatedElements = flushPendingDrag();
        const updatedRuns = recalcCablesForElement(dragId, updatedElements);
        saveHistory({ elements: updatedElements, cables: updatedRuns });
    }
  ...
  const recalcCablesForElement = (elId: string, elements = latestElementsRef.current) => {
      const localElementById = new Map(elements.map(element => [element.id, element] as const));
      const movedEl = localElementById.get(elId);
      if (!movedEl) return cableRuns;
      
      const updatedRuns = cableRuns.map(run => {
          if (run.fromId === elId || run.toId === elId) {
              const otherId = run.fromId === elId ? run.toId : run.fromId;
              const otherEl = localElementById.get(otherId);
              if (otherEl) {
  ...
  const getElementColor = (el: PlanElement): string => {
    if (viewMode === 'voltage' && el.type !== 'db') {
        const du = voltageDropByElementId.get(el.id) ?? 0;
        ...
        >
          ...
          >
            ...
            {floorPlanElements.map((el) => {
              ...
              const dU = viewMode === 'voltage' ? (voltageDropByElementId.get(el.id) ?? 0) : 0;
